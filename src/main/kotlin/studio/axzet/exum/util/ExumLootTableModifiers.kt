    ...
    companion object {
        ...
        fun modifyLootTable() {
            LootTableEvents.MODIFY.register(Modify { _: ResourceManager?, _: LootManager?, id: Identifier, tableBuilder: LootTable.Builder, _: LootTableSource? ->
                ...
                when(id) {
                    ABANDONED_MINESHAFT_CHEST_ID -> tableBuilder.pool(customPoolBuilder(1f, 0.3f, ExumItems.INCANTATIO))
                    BASTION_TREASURE_CHEST_ID    -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                    DESERT_PYRAMID_CHEST_ID      -> tableBuilder.pool(customPoolBuilder(1f, 0.5f, ExumItems.INCANTATIO))
                    STRONGHOLD_LIBRARY_CHEST_ID  -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                    JUNGLE_TEMPLE_CHEST_ID       -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                    PILLAGER_OUTPOST_CHEST_ID    -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                    MANSION_CHEST_ID             -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                    IGLOO_STRUCTURE_CHEST_ID     -> tableBuilder.pool(defaultPoolBuilder(ExumItems.INCANTATIO))
                }
